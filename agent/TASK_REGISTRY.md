...
The registry is the execution map for the project. Task IDs are stable. Execute in order unless a dependency explicitly blocks the next task.
...
- 13-04 Connectivity/sync status and safe retry
- 13-05 Phase 13 gate
...
Each task must have a specification before implementation. The implementation agent must create or update the corresponding execution record with acceptance results and evidence. A task is not complete because code exists; the task gate must pass.
