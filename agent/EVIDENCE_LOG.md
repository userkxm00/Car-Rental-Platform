...
## Checkpoint: 09-B — Financial Ledger / Invoices (transaction model · invoices · immutable events · reconciliation)

- Workstream: `09-B`
- Task range: `09-B01` (transaction model) · `09-B02` (invoice model) · `09-B03` (invoice items) · `09-B04` (payment history) · `09-B05` (balance calculations) · `09-B06` (immutable financial events) · `09-B07` (reconciliation view)
- Status: `DONE`
- Date: `2026-09-15`
- Summary: (1) 09-B01: migration #27 `20260904160000_financial_ledger` — append-only `ledger_transactions` (kind PAYMENT_CONFIRMED/PAYMENT_VOIDED/DEPOSIT_HELD/DEPOSIT_RELEASED/INVOICE_ISSUED/INVOICE_VOIDED, currency, amountMinor, sourceType/sourceId, actorUserId, tenant+booking indexes) + `invoices` (per-tenant `invoiceNumber` unique, partial unique `invoices_booking_active_key` so at most one ISSUED invoice per booking) + `invoice_items`. (2) 09-B02/09-B03: `POST …/bookings/:bookingId/invoices` composes items server-side from the immutable `booking_price_snapshots.pricingJson` (DEPOSIT line when depositMinor > 0 + RENTAL remainder — never client totals), numbers `INV-{bookingNumber}-{seq:003}` from the tenant invoice count, gated by the payment-eligibility status window (409 `BILLING_BOOKING_NOT_ELIGIBLE` otherwise); correction is void-and-reissue (active invoice flipped to VOIDED with `INVOICE_VOIDED` + fresh `INVOICE_ISSUED` rows in one transaction) and explicit `POST …/invoices/:invoiceId/void` keeps the historical row — double void → 409 `BILLING_INVOICE_STATE`. (3) 09-B04/09-B05: `GET …/payments` continues to project the pay-at-agency state (paid/outstanding derived from CONFIRMED records against the snapshot — no mutable balance counters) and `GET …/ledger`/`GET …/invoices` expose the booking payment history; me-portal mirrors through the booking-customer binding. (4) 09-B06: the ledger has no HTTP write path (append goes through domain services only); the 09-A pipeline emits DEPOSIT_HELD at intent creation, PAYMENT_CONFIRMED at confirmation (`LedgerService` inside the same transaction), PAYMENT_VOIDED at pending-void, DEPOSIT_RELEASED at release; invoice issuance/voiding emits its own events; nothing updates or deletes a posted row. (5) 09-B07: `GET …/bookings/:bookingId/finance` (BILLING_READ) — the staff reconciliation view re-derives every money fact from raw sources and cross-checks intent-vs-records settlement status, intent-vs-snapshot totals, active-invoice-vs-snapshot and per-fact ledger event completeness; out-of-band writes (e.g. a raw CONFIRMED row without its event) return `reconciled: false` with the failing check named, never silent absorption. (6) Permissions per docs/37: `billing.read` for owner/manager/agent/finance, `billing.manage` for owner/admin/finance — STAFF_AGENT 403 on issue/void, FINANCE issues and continues the sequence. Staff reads hit `bookingExistsInTenant` first so foreign bookings 404 (not a silent empty array). (7) Shared `parseBookingTotals` moved to `pricing/domain/booking-totals.ts` — contracts, payments and billing share one snapshot-totals semantics. api-client: `createBillingApi`/`createMeBillingApi` + finance DTO.
- Environment: fourth sandbox reset (branch back to `e534d20`, node_modules/PostgreSQL/tokens wiped; HEAD intact on origin at `5161197`) — recovered with the established recipe (`git fetch origin arena/…` + `git reset --mixed FETCH_HEAD` preserved every uncommitted 09-B edit, `npm install`, `local-pg start`, `car_rental_preview` re-created via node-pg, `migrate deploy` #1…#27 on both DBs, `npm run db:generate`, dev-jwks :5433 + preview API :4000 restarted with fresh `.dev-token`s).
- Repairs (this checkpoint): (1) Post-reset full regression surfaced two wall-clock-rotted specs — `quotes.service.spec` hardcoded `2026-09-10` behind the future gate and `availability.e2e-spec` anchored hold-expiry windows to the same fixed day; both converted to `Date.now()`-relative fixtures preserving the offset-normalization and inert-vs-live assertions (`b06d122`). (2) `quotes.service.spec.ts` pre-existing lint debt (untyped mock params / async-without-await / unbound-method expectation — from the `490f522` known-debt list) cleaned in the same commit. (3) Live smoke caught the ledger response DTO dropping `currency` (select/mapper/row type/api-client, `d824af4`).
- Verification: API typecheck 0; eslint 0 on every touched TS file (billing module incl. specs, payments/portal/authorization wiring, booking-totals, billing e2e, api-client); unit 644/644 (53 suites — billing domain 21 incl. 9 reconciliation, billing.service 17); e2e 266/266 (32 suites — `billing` 12 over JWKS 4174: empty ledger, DEPOSIT_HELD+PAYMENT_CONFIRMED emission through the 09-A pipeline, snapshot-composed issuance, void-and-reissue preserving history, eligibility window, permission boundary, cross-tenant 403/404, me-portal mirrors, no-HTTP-write 404, reconciliation coherent + out-of-band drift flagged, anonymous 401).
- Live preview smoke `scripts/qa-09b-billing-smoke.cjs` (33 checks over HTTP, `car_rental_preview` at migration #27): read-only ledger 404-on-POST, empty→DEPOSIT_HELD→PAYMENT_CONFIRMED chain with amounts, `INV-…-001/002/003` sequencing, supersede + explicit void trails with double-void 409, cancelled-booking 409, STAFF_AGENT read/403-manage, FINANCE manage, reconciliation coherent→drift-flagged→coherent-again, me-portal mirrors + stranger 404, foreign-agency 403, cross-tenant 404, unauthenticated 401.
- Commit: `660d863` (schema/migration #27) + `0fe92b9` (feat 09-B) + `b06d122` (wall-clock spec repairs) + `d824af4` (currency projection + smoke) + `32b6fda` (feat 09-B07 reconciliation) — pushed to origin
